...
export class RegisterComponent implements OnInit {
  ...
  ngOnInit() {
    this.forma = new FormGroup({
      ...
      emailconfirm: new FormControl(null, [Validators.email, Validators.required]),
  ...
  register() {
    ...
    console.log(this.datasave);
      ...
      this.registerservice.createcompania(this.datasave).subscribe( (resp:any) => {
        console.log(resp);     
       if(resp.success){
         swal.fire('Muy bien!',"Te has  registrado con éxito, verifica tu email para activar tu cuenta",'success');
       ...
       if(err.status === 400){
         this.errors = err.error.errors as string[];
          ...
          swal.fire('Algo salió mal!','Error al crear cuenta, intente nuevamente','error');
       }
